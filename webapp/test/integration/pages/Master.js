				...
				actions : {
										...
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("CandidateID") === oObject.CandidateID;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].CandidateID;
										} else {
											sCurrentId = aItemsNotInTheList[0].CandidateID;
										}
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("CandidateID"),
							title: oBindingContext.getProperty("CandidateName")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "CandidateSet (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'CandidateSet (0)'");
							},
					...
					theListShouldBeSortedAscendingOnUnitNumber : function () {
						return this.theListShouldBeSortedAscendingOnField("TotalPoints");
					},

					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("CandidateName");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all CandidateSet to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "CandidateSet (" + iExpectedLength + ")"}),
									success : function () {
