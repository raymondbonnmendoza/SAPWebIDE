sap.ui.define([
	...
	return BaseObject.extend("com.sap.interview_summary.model.GroupSortState", {
		 ...
		 * @alias com.sap.interview_summary.model.GroupSortState
		 */
		...
		/**
		 * Sorts by CandidateName, or by TotalPoints
		 *
		...
		/**
		 * Groups by TotalPoints, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "TotalPoints") {
				...
				this._oViewModel.setProperty("/sortBy", "TotalPoints");

				aSorters.push(
					new Sorter("TotalPoints", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "CandidateName");
			}
