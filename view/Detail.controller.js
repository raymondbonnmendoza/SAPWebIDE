...
com.sap.open.offline.util.Controller.extend("com.sap.open.offline.view.Detail", {
	/**
	...
	onInit: function() {
		...
		if (!this._detailReadMode) {
			...
			this._detailReadMode = sap.ui.xmlfragment(this._detailReadModeFragmentName, "com.sap.open.offline.view.DetailReadMode", this);
			this.switchMode("read");
		}

		if (!this._detailEditMode) {
			this._detailEditModeFragmentName = this.getView().getId() + "detailEditMode";
			this._detailEditMode = sap.ui.xmlfragment(this._detailEditModeFragmentName, "com.sap.open.offline.view.DetailEditMode", this);
		}

		if (!this._detailCreateMode) {
			this._detailCreateModeFragmentName = this.getView().getId() + "detailCreateMode";
			this._detailCreateMode = sap.ui.xmlfragment(this._detailCreateModeFragmentName, "com.sap.open.offline.view.DetailCreateMode", this);
		}
	...
	openDialog: function(sI18nKeyword) {
		if (!this._dialog) {
			...
			this._dialog = sap.ui.xmlfragment(frgId, "com.sap.open.offline.view.MsgDialog", this);
	...
	openCancelConfirmDialog: function() {
		if (!this._cancelConfirmDialog) {
			...
			this._cancelConfirmDialog = sap.ui.xmlfragment(frgId, "com.sap.open.offline.view.CancelConfirmDialog", this);
			...
			if (oParameters.arguments.tab === "AddItem") {
				...
				if (oIconTabBar) {
					oIconTabBar.getItems().forEach(function(oItem) {
						oItem.bindElement(com.sap.open.offline.util.Formatter.uppercaseFirstChar(oItem.getKey()));
					});
	...
	showEmptyView: function() {
		this.getRouter().myNavToWithoutHash({
			...
			targetViewName: "com.sap.open.offline.view.NotFound",
			targetViewType: "XML"
		});
	},
	...
	openDeleteConfirmDialog: function() {
		if (!this._deleteConfirmDialog) {
			...
			this._deleteConfirmDialog = sap.ui.xmlfragment(frgId, "com.sap.open.offline.view.DeleteConfirmDialog", this);
